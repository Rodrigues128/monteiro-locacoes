...
export default function ProductCard({ product, onAdd, onDetails, added }) {
  ...
  return <article className="group flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
    <button onClick={() => onDetails(product)} className="relative block h-56 w-full overflow-hidden bg-gray-50 text-left">
      <Image src={product.image} alt={product.name} className="h-full w-full transition duration-500 group-hover:scale-105" fittingType="fit"/>
    ...
    <div className="flex flex-1 flex-col p-6">
      ...
      <ul className="mt-4 space-y-2">{product.features.map(f=><li key={f} className="flex items-center gap-2.5 text-sm text-gray-600"><span className="h-2 w-2 shrink-0 rounded-full bg-[#00BFFF]"/>{f}</li>)}</ul>
      <div className="mt-5 flex items-end justify-between"><div><p className="text-xs text-gray-400">A partir de</p><p className="text-2xl font-black text-gray-900">R$ {product.price},00</p></div></div>
      <div className="mt-5 flex gap-2">
        ...
        <a href={waLink} target="_blank" rel="noreferrer" className="flex flex-[1.4] items-center justify-center gap-2 rounded-full bg-[#25D366] py-3 text-sm font-bold text-white transition hover:brightness-110"><MessageCircle size={17}/> Falar no WhatsApp</a>
      </div>
    </div>
  </article>;
}
