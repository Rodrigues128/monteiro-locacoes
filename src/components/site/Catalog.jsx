...
export default function Catalog(/** @type {CatalogProps} */ { onAdd, addedIds }) {
  ...
  useEffect(() => {
    fetchProducts().then(setProducts).catch(() => setError("Não foi possível carregar o catálogo agora.")).finally(() => setLoading(false))
  }, [])
  ...
  return <section id="atracoes" className="bg-[#F9FAFB] px-5 py-24 lg:px-8">
    <div className="mx-auto max-w-7xl"><div className="mx-auto mb-12 max-w-2xl text-center"><p className="mb-3 text-sm font-black uppercase tracking-[.2em] text-[#00BFFF]">Catálogo</p><h2 className="text-4xl font-black tracking-[-.03em] text-gray-900 sm:text-5xl">Nossas Atrações</h2><p className="mt-4 text-lg text-gray-500">Tudo o que sua festa precisa, selecionado, higienizado e montado pela nossa equipe.</p></div>
      {!loading && products.length > 0 && <div className="mb-10 flex flex-wrap justify-center gap-2">{categories.map((item) => <button key={item} onClick={() => setCategory(item)} className={`whitespace-nowrap rounded-full px-5 py-2.5 text-sm font-bold transition ${category === item ? "bg-[#00BFFF] text-white shadow-md" : "bg-white text-gray-500 shadow-sm hover:text-gray-900"}`}>{item}</button>)}</div>}
      {loading && <p className="py-14 text-center text-gray-500">Carregando atrações...</p>}
      {error && <p className="rounded-2xl bg-red-50 p-5 text-center text-red-700">{error} Verifique a configuração do Supabase.</p>}
      {!loading && !error && !products.length && <p className="py-14 text-center text-gray-500">Ainda não há atrações disponíveis no catálogo.</p>}
    ...
    </div><ProductDetail product={selected} onClose={() => setSelected(null)} onAdd={(product) => { onAdd(product); setSelected(null) }} />
  </section>
}
