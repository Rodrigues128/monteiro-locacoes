      ...
      >
        <div className="bg-gradient-to-br from-[#00BFFF] to-[#008fc0] px-6 py-7 text-white sm:px-8">
          ...
          <p className="mt-3 text-sm leading-relaxed text-white/85">
            A reserva será registrada na agenda e ficará disponível para sua
            equipe acompanhar.
          </p>
        </div>
        ...
        <div className="space-y-5 p-6 sm:p-8">
          <div className="rounded-2xl bg-slate-50 p-4">
            ...
            <p className="mt-2 text-sm font-medium text-slate-500">
              {eventDate || "Data a definir"} · {items.length}{" "}
          ...
          <div>
            ...
            <div className="mt-3 max-h-36 space-y-2 overflow-y-auto pr-1">
              {items.map((item, index) => (
                <div
                  key={`${item.name}-${index}`}
                  className="flex items-center justify-between gap-4 rounded-xl border border-slate-100 px-3 py-2.5"
                >
                  <span className="truncate text-sm font-bold text-slate-700">
                    {item.name}
                  </span>
                  <span className="shrink-0 rounded-lg bg-cyan-50 px-2 py-1 text-xs font-black text-cyan-800">
                    {item.quantity}×
                  </span>
                </div>
              ))}
...
function MessageList(
  /** @type {MessageListProps} */ { messages, selectedId, onSelect },
) {
  return (
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span
                    className={`rounded-full px-2 py-1 text-[10px] font-black ${ready ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
                  >
                    {ready ? "PRONTO" : "REVISAR"}
                  </span>
                </div>
                <p className="mt-1 text-sm text-slate-500">
                  {data.event?.date || "Sem data"} ·{" "}
                  {data.event?.venue_type || "Local não informado"}
                </p>
...
function Editor(
  ...
  const [draft, setDraft] = useState(
    /** @type {AppointmentDraft} */ message.extracted_data || {},
  );
  const [items, setItems] = useState(
    /** @type {MessageItem[]} */ message.appointment_message_items || [],
  );
    ...
    try {
      ...
      setNotice(
        status === "ready_to_confirm"
          ? "Revisão salva. A reserva está pronta para confirmação."
          : "Revisão salva. Complete os campos pendentes para prosseguir.",
      );
      onSaved();
    } catch (error) {
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button
          onClick={suggest}
          className="inline-flex items-center gap-2 rounded-xl border border-cyan-200 bg-cyan-50 px-4 py-2.5 text-sm font-bold text-[#008fc0]"
        >
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        <Field
          ...
          onChange={(value) => change("client", "name", value)}
        />
        <Field
          label="CPF/CNPJ"
          value={draft.client?.document_number}
          onChange={(value) =>
            change("client", "document_number", value.replace(/\D/g, ""))
          }
        />
        <Field
          label="Telefone"
          ...
          onChange={(value) =>
            change("client", "phone", value.replace(/\D/g, ""))
          }
        />
        <Field
          ...
          onChange={(value) => change("event", "date", value)}
        />
        <Field
          label="Horário inicial"
          type="time"
          value={draft.event?.start_time}
          onChange={(value) => change("event", "start_time", value)}
        />
        <Field
          label="Horário final"
          type="time"
          value={draft.event?.end_time}
          onChange={(value) => change("event", "end_time", value)}
        />
        <Field
          label="Endereço"
          value={draft.event?.address}
          onChange={(value) => change("event", "address", value)}
        />
        <Field
          label="Local do evento"
          value={draft.event?.venue_type}
          onChange={(value) => change("event", "venue_type", value)}
        />
        <Field
          label="Valor total"
          type="number"
      ...
      <div className="mt-8 border-t border-slate-100 pt-6">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Vincule cada produto identificado na mensagem e informe a quantidade.
        </p>
        <div className="mt-4 space-y-3">
          {items.map((item) => (
            <div
              key={item.id}
              className="grid gap-3 rounded-2xl bg-slate-50 p-4 md:grid-cols-[1fr_110px_1.2fr]"
            >
              <div>
              ...
              <label className="block text-sm font-bold text-slate-700">
                ...
                <select
                  ...
                  onChange={(event) => changeItem(item.id, event.target.value)}
      ...
      {issues.length > 0 && (
        <div className="mt-6 rounded-2xl border border-amber-100 bg-amber-50 p-4">
          <p className="flex items-center gap-2 font-bold text-amber-800">
            <TriangleAlert size={18} /> Pendências para confirmação
          </p>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl bg-[#00BFFF] px-5 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"
        >
          <CalendarCheck2 size={17} /> Confirmar reserva
        </button>
        {notice && (
          <p className="text-sm font-semibold text-slate-600">{notice}</p>
        )}
      </div>
      <ConfirmationModal
        open={confirming}
...
export default function AppointmentReview() {
  ...
  const [selected, setSelected] = useState(
    /** @type {AppointmentMessage | null} */ null,
  );
    ...
    try {
      const [messagesResult, productsResult, aliasesResult] = await Promise.all(
        [
          supabase
            .from("appointment_messages")
            .select("*, appointment_message_items(*)")
  ...
  if (!isSupabaseConfigured)
    return (
      <main className="grid min-h-screen place-items-center p-5">
        Configure o Supabase para usar a fila.
      </main>
    );
  if (!session)
    return (
    ...
    <AdminWorkspace>
      <div className="mx-auto max-w-7xl">
        <div className="mt-5 flex flex-wrap items-center justify-between gap-4">
          ...
          >
            <ArrowLeft size={16} /> Importar outra mensagem
          </Link>
          <Link
            to="/admin/reservas"
        ...
        <header className="mt-8">
          ...
          <p className="mt-3 max-w-2xl leading-relaxed text-slate-500">
            Corrija os dados, vincule os produtos e confirme apenas quando a
            reserva estiver pronta.
          </p>
        </header>
        {error ? (
          <p className="mt-8 rounded-2xl bg-red-50 p-4 text-red-700">{error}</p>
        ) : (
          <div className="mt-8 grid gap-6 lg:grid-cols-[340px_1fr]">
            ...
            {selected ? (
              ...
              <section className="grid min-h-80 place-items-center rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-center">
                <div>
                  <ClipboardList className="mx-auto text-[#00BFFF]" size={32} />
                  <h2 className="mt-4 text-xl font-black">Fila vazia</h2>
                  <p className="mt-2 text-slate-500">
                    Importe uma mensagem para começar a revisão.
                  </p>
                  <Link
                    to="/admin/agendamentos"
